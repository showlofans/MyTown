...
import com.mycountry.adapter.TourAdapter;
import com.mycountry.details.TourDetail;
...
public class TourService extends Activity {
	private static final String TAG = "TourService";// 定义当前标签
	private ListView lv;
	private static List<Tour> list;
	private Handler handler;
	private TourAdapter adapter;
	private String et_title="tour";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(TourService.this,
						TourDetail.class);
				intent.putExtra("tour", adapter.getItem(position));
		...
		plus.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(TourService.this, AddFruit.class);
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				adapter = new TourAdapter(TourService.this, list);
	...
	private List<Tour> jxJSON(String result) {
		List<Tour> ll = new ArrayList<Tour>();
		if (result == null) {
			return null;
		}
		try {
			...
			for (int i = 0; i < jsonarr.length(); i++) {
				...
				String tour_title = obj.getString("tour_title");
				String tour_price = obj.getString("tour_price");
				String tour_sales = obj.getString("tour_sales");
				String tour_distance = obj.getString("tour_distance");
				byte[] pic = PageService.getImage(Constant.aURL
						+ obj.getString("tour_picture"));
				Tour sa = new Tour(tour_title, tour_price,
						tour_sales, tour_distance, pic);
				ll.add(sa);
			}
			return ll;
		} catch (Exception e) {
