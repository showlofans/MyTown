...
import com.mycountry.adapter.Push3Adapter;
...
public class Push3Service extends Activity implements OnClickListener {
	private static final String TAG = "Push3Service";// 定义当前标签
	private ListView lv;
	private List<Push3> list;
	private Handler handler;
	private Push3Adapter adapter;
	public String x;
	private String et_title="push3";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(Push3Service.this, MeatDetail.class);
				intent.putExtra("push3", adapter.getItem(position));
		...
		plus.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(Push3Service.this, AddFruit.class);
				intent.putExtra("fromtype", et_title);
				startActivity(intent);
			}
		});
		list = new ArrayList<Push3>();
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				adapter = new Push3Adapter(Push3Service.this, list);
	...
	private List<Push3> jxJSON(String result) {
		List<Push3> ll = new ArrayList<Push3>();
		if (result == null) {
			return null;
		}
		try {
			...
			for (int i = 0; i < jsonarr.length(); i++) {
				...
				String title = obj.getString("push3_title");
				String content = obj.getString("push3_price");
				String sales = obj.getString("push3_sales");
				String price = obj.getString("push3_price");
				String location = obj.getString("country_name");
				String distance = obj.getString("push3_distance");
				byte[] pic = PageService.getImage(Constant.aURL
						+ obj.getString("push3_picture"));
				Push3 sa = new Push3(title, content, sales, price, location, distance, pic);
				ll.add(sa);
			}
			return ll;
		} catch (Exception e) {
