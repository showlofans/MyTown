...
import com.mycountry.adapter.WineAdapter;
import com.mycountry.details.WineDetail;
...
public class WineService extends Activity {
	private static final String TAG = "WineService";// 定义当前标签
	private ListView lv;
	private static List<Wine> list;
	private Handler handler;
	private WineAdapter adapter;
	private TextView title;
	public String x;
	private String et_title = "wine";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(WineService.this, WineDetail.class);
		...
		plus.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(WineService.this, AddFruit.class);
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				adapter = new WineAdapter(WineService.this, list);
	...
	private List<Wine> jxJSON(String result) {
		List<Wine> ll = new ArrayList<Wine>();
		if (result == null) {
			return null;
		}
		try {
			...
			for (int i = 0; i < jsonarr.length(); i++) {
				...
				String wine_title = obj.getString("wine_title");
				String wine_price = obj.getString("wine_price");
				String wine_sales = obj.getString("wine_sales");
				String wine_distance = obj.getString("wine_distance");
				byte[] pic = PageService.getImage(Constant.aURL
						+ obj.getString("wine_picture"));
				Wine sa = new Wine(wine_title, country_name, wine_price,
						wine_sales, wine_distance, pic);
				ll.add(sa);
			}
			return ll;
		} catch (Exception e) {
