...
import com.mycountry.adapter.FruitAdapter;
import com.mycountry.details.FruitDetail;
...
public class FruitService extends Activity {
	private static final String TAG = "FruitService";// 定义当前标签
	private ListView lv;
	private static List<Fruit> list;
	private Handler handler;
	public String et_title = "fruit";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(FruitService.this, FruitDetail.class);
				intent.putExtra("fruit", adapter.getItem(position));
		...
		plus.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(FruitService.this, AddFruit.class);
		...
		if (NetUtils.isNetworkConnected(FruitService.this)) {
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				adapter = new FruitAdapter(FruitService.this, list);
	...
	private List<Fruit> jxJSON(String result) {
		List<Fruit> ll = new ArrayList<Fruit>();
		if (result == null) {
			return null;
		}
		try {
			...
			for (int i = 0; i < jsonarr.length(); i++) {
				...
				String fruit_title = obj.getString("fruit_title");
				String country_name = obj.getString("country_name");
				String fruit_price = obj.getString("fruit_price");
				String fruit_sales = obj.getString("fruit_sales");
				String fruit_content = obj.getString("fruit_content");
				byte[] pic = PageService.getImage(Constant.aURL
						+ obj.getString("fruit_picture"));
				Fruit sa = new Fruit(fruit_title, country_name, fruit_price,
						fruit_sales, fruit_content, pic);
				ll.add(sa);
			}
			return ll;
		} catch (Exception e) {
