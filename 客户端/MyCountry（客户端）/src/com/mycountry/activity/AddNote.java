...
public class AddNote extends Activity implements OnClickListener {
	...
	final String url = Constant.addnoteURL;//路径
	private String path = "";
	private String name_edit;
	private EditText nameET, passwordET;

	private GridAdapter adapter;
	String note_picture="";// 发送图片的string字符串
	File[] fileArray = null;
	TextView send;
		...
		*/
		...
		image.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if (arg2 == Bimp.bmp.size()) {
					new PopupWindows(AddNote.this, image);
				} else {
		...
		up.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				note_title = nameET.getText().toString();
				final EditText passwordET = (EditText) findViewById(R.id.note_content);
				note_content = passwordET.getText().toString();
				if (note_title.matches("")) {
					Toast.makeText(AddNote.this, "请输入标题", Toast.LENGTH_SHORT)
							.show();
				} else if (note_content.matches("")) {
					Toast.makeText(AddNote.this, "请输入详细内容", Toast.LENGTH_SHORT)
							.show();
				}
						...
						if (null != fileArray) {
							...
							String picfile = File2StringUtil
									.file2String("/mnt/sdcard/MyCountry/"
											+ fileArray[0].getName());
							note_picture= picfile;
							//传递图片
							params.put("note_picture", note_picture);
						}}
		...
		hd = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				Toast.makeText(AddNote.this, msgStr, Toast.LENGTH_SHORT).show();
