...
import com.mycountry.service.NoteService;
...
public class TourServlet extends HttpServlet {
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List<Tour> tours = new ArrayList(); // 返回JoinService的数据
			TourService dao = new TourService();
			tours = dao.find();
			Gson gson = new Gson();
			String x = gson.toJson(tours);
			// StringBuffer sb = new StringBuffer();// 数据缓存
			// sb.append('[');

			// for (Pifa pifa : pifas) {//
			// person.getName(),person.getAddress()获得姓名，地址，一些符号转换为char
